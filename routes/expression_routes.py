...
from generators.expression_types import generators_expression
import random
...
def generate_problems_and_answers(n=20):
    problems = []
    for _ in range(n):
        gen = random.choice(generators_expression)
        p, a = gen()
        problems.append({"problem": p, "answer": a})
    return problems

@router.get("/", response_class=HTMLResponse)
async def expression_page():
    return """
    <!DOCTYPE html>
    <html>
    <head><meta charset="UTF-8"><title>四則計算プリント生成</title></head>
    <body>
    <h1>四則計算プリント生成</h1>
...
@router.get("/generate")
def expression_generate(n: int = 20):
...
def expression_pdf(filename="fraction.pdf"):
    ...
    c.drawCentredString(width/2, height-40, "四則計算プリント（問題）")
    y = height - 80
    ...
    c.showPage()
    c.setFont("HeiseiMin-W3", 12)
    c.drawCentredString(width/2, height-40, "四則計算プリント（解答）")
    y = height - 80
    for i, item in enumerate(current_problems):
