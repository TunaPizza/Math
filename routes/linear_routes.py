...
from generators.linear_types import generators_linear
import random
...
def generate_problems_and_answers(n=20):
    problems = []
    for _ in range(n):
        gen = random.choice(generators_linear)
        p, a = gen()
...
async def linear_page():
    ...
    <!DOCTYPE html>
    <html>
    <head><meta charset="UTF-8"><title>一次関数プリント生成</title></head>
    <body>
    <h1>一次関数プリント生成</h1>
    ...
    generateBtn.addEventListener('click', async()=>{const n=numInput.value;const res=await fetch(`/linear/generate?n=${n}`);const data=await res.json();let text="";data.forEach((p,i)=>{text+=(i+1)+". "+p.problem+"\\n"});preview.textContent=text});
    pdfBtn.addEventListener('click',()=>{const n=numInput.value;window.open(`/linear/pdf?n=${n}`,'_blank')});
    </script>
    </body>
    </html>
    """
