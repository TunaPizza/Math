...
from generators.algebra_types import generators_algebra
import random
...
def generate_problems_and_answers(n=20):
    problems = []
    for _ in range(n):
        gen = random.choice(generators_algebra)
        p, a = gen()
        problems.append({"problem": p, "answer": a})
    return problems

@router.get("/", response_class=HTMLResponse)
async def algebra_page():
    return """
    <!DOCTYPE html>
    <html>
    <head><meta charset="UTF-8"><title>文字式プリント生成</title></head>
    <body>
    <h1>文字式プリント生成</h1>
...
def algebra_pdf(filename="algebra.pdf"):
    ...
    c.drawCentredString(width/2, height-40, "文字式プリント（問題）")
    y = height - 80
    ...
    c.showPage()
    c.setFont("HeiseiMin-W3", 12)
    c.drawCentredString(width/2, height-40, "文字式プリント（解答）")
    y = height - 80
    for i, item in enumerate(current_problems):
